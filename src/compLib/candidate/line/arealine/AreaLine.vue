		...
		methods:{
			option(){
				...
				data.legends.forEach(function(legend){
					...
					let styleSeries = style.series,currentSerie = null;
					...
					series.push({
						...
						markPoint:{
							data:[
								(currentSerie == null || !currentSerie.max)?{}:{type:'max',name:'最大值'},
								(currentSerie == null || !currentSerie.min)?{}:{type:'min',name:'最小值'}
							],
							
						},

						markLine:{
							data:(currentSerie == null || !currentSerie.average)?[]:[{type:'average',name:'平均值'}]
						}
					})
