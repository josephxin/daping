    ...
    methods: {
        ...
        createMultipleY(options,style){
            ...
            if(style.multipleY.configs.length){
                style.multipleY.configs.forEach((item,index) => {
                    ...
                    if(style.series[index] && style.series[index].inverse) option.inverse = true
                    yAxis.push(option)
                })
                return yAxis
            }
        ...
        option() {
            ...
            for (let i = 0; i < dimensions.length - 1; i++) {
                ...
                let opt = {
                    type,
                    stack: style.series[i] ? (style.series[i].stack ? style.series[i].stack : i + 1) : i+10 + 1,
                    seriesLayoutBy: style.seriesLayoutBy,
                    itemStyle: {
                        barBorderRadius: [style.series[i] ? style.series[i].barRadius : 0, style.series[i] ? style.series[i].barRadius : 0, 0, 0]
                    },
                    hoverAnimation: true,
                    barWidth: style.barWidth,
                    name: dimensions[i + 1],
                }
                if(style.showArea) opt.areaStyle = {}
                opt.smooth = style.smooth
                opt.lineStyle = {
                ...
                if (style.series[i]) {
                    opt.type = style.series[i].type
                    opt.smooth = style.series[i].smooth ? style.series[i].smooth : style.smooth,
                    opt.symbol = style.series[i].symbol
                    opt.symbolSize = style.series[i].smoothSize
                    opt.lineStyle = {
                        color: style.series[i].color,
                        width: style.series[i].lineWidth
                    }
                    opt.yAxisIndex = style.series[i].yAxisIndex
                    if (style.series[i].showArea) {
                        opt.areaStyle = {}
                        if (style.series[i].gradient && style.series[i].gradient.length) {
                            opt.areaStyle.color = {
                                type: 'linear',
                                x: 0,
                                y: 0,
                                x2: 0,
                                y2: 1,
                                colorStops: style.series[i].gradient,
                                global: false
                            }
                        }
                    }
            ...
            if(style.y.showMaxMin){
                option.yAxis[0].min = style.y.min
                option.yAxis[0].max = style.y.max
            }
