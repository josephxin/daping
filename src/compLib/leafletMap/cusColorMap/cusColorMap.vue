    ...
    methods: {
        ...
        async convertChartData(data) {
            if (this.compConf.data.datasource.type === 'dataset') {
                ...
                this.data = await this.loadGisServer('Region');
                ...
                if (dataConf && dataConf.layerObj && dataConf.layerObj.length > 0) {
                    this.gisData = await this.loadGisServer('Region');
                }
        ...
        notifyBase() {
            if (this.gisBase) {
                this.layerObj = this.gisBase.addProLayer(this.data, 'optimiseColor', this.compConf.style);
                if (this.layerObj) {
